...
//    MARK: - Send Notification
    func sendNotifications(
        ...
        guard !user.deviceTokens.isEmpty else { throw Abort(.notFound) }
                
        for deviceToken in (try? await user.$deviceTokens.get(on: app.db)) ?? .init() {
            app.apns.send(.init(title: message), to: deviceToken.value).whenComplete {
                switch $0 {
                case .success():
                    print("❕NOTIFICATION: notiffication sent.")
...
//    MARK: Send Notification Subscriptions
    func sendNotificationSubscriptions(
        ...
        for sub in try await self.getSubscription(app, with: .nextDayExpiration) {
            if isThrowEnable {
                try await self.sendNotifications(app, subscription: sub, localizedMessages: localizedMessages)
            } else {
                try? await self.sendNotifications(app, subscription: sub, localizedMessages: localizedMessages)
            }
        }
